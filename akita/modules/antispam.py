...
import akita.modules.sql.global_bans_sql as sql
from akita import dispatcher, OWNER_ID, SUDO_USERS, DEV_USERS, SUPPORT_USERS, WHITELIST_USERS, STRICT_GBAN, GBAN_LOGS
from akita.modules.helper_funcs.chat_status import user_admin, is_user_admin, support_plus, dev_plus, sudo_plus
from akita.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from akita.modules.helper_funcs.misc import send_to_list
from akita.modules.sql.users_sql import get_all_chats
...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text("There is no way I can gban this user. He is one of my owners!")
        return

    if int(user_id) in SUDO_USERS:
        message.reply_text("I spy, with my little eye... a Sudo User! Why are you guys turning on each other?")
        return
    ...
    if int(user_id) in WHITELIST_USERS:
        message.reply_text("Whitelisted users cannot be gbanned!")
        return
...
def gbanstat(bot: Bot, update: Update, args: List[str]):
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text("I've disabled antispam security in this group. You won't be protected"
                                                "anymore.")
    else:
        update.effective_message.reply_text("Give me some arguments to choose a setting! on/off, yes/no!\n\n"
                                            "Your current setting is: {}\n"
                                            "When True, any antispam bans that happen will also happen in your group. "
                                            "When False, they won't, leaving you at the possible mercy of "
                                            "spammers or trolls".format(sql.does_chat_gban(update.effective_chat.id)))
...
__mod_name__ = "Anti-Spam Security"
