...
from akita import dispatcher, OWNER_ID, SUDO_USERS, DEV_USERS, WHITELIST_USERS, GBAN_LOGS, LOGGER, spamfilters
from akita.modules.helper_funcs.handlers import CMD_STARTERS
from akita.modules.helper_funcs.misc import is_module_loaded, send_to_list
from akita.modules.helper_funcs.chat_status import is_user_admin
from akita.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from akita.modules.helper_funcs.string_handling import markdown_parser
from akita.modules.disable import DisableAbleCommandHandler

import akita.modules.sql.feds_sql as sql

from akita.modules.connection import connected
from akita.modules.helper_funcs.alternate import send_message
...
def join_fed(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			message.reply_text("Failed to join federation! Please contact @DraXRobotsSupport should this problem persists!")
			return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
	...
	if fban:
		...
		if not x:
			message.reply_text("Failed to ban from the federation! If this problem continues, contact @DraXRobotsSupport.")
			return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			update.effective_message.reply_text("Big F! There is an error while setting federation rules! If you wondered why please ask it in @DraXRobotsSupport !")
			return
