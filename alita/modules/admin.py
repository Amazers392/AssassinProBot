...
import alita.modules.sql.users_sql as sql

from alita import dispatcher, TOKEN
from alita.modules.disable import DisableAbleCommandHandler
from alita.modules.helper_funcs.chat_status import bot_admin, can_promote, user_admin, can_pin, connection_status
...
def set_title(bot: Bot, update: Update, args: List[str]):
    ...
    if status is True:
        bot.sendMessage(chat.id, f"Sucessfully set title for {mention_html(user_member.user.id, user_member.user.first_name)} "
