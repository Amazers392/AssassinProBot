...
import alita.modules.sql.notes_sql as sql
from alita import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP, DEV_USERS
from alita.__main__ import DATA_IMPORT
from alita.modules.helper_funcs.chat_status import user_admin
from alita.modules.helper_funcs.misc import build_keyboard, revert_buttons
from alita.modules.helper_funcs.msg_types import get_note_type
from alita.modules.rules import get_rules
from alita.modules.sql import notes_sql as notesql
import alita.modules.sql.rules_sql as rulessql
from alita.modules.sql import warns_sql as warnssql
import alita.modules.sql.blacklist_sql as blacklistsql
from alita.modules.sql import disable_sql as disabledsql
from alita.modules.sql import cust_filters_sql as filtersql
import alita.modules.sql.welcome_sql as welcsql
import alita.modules.sql.locks_sql as locksql
from alita.modules.connection import connected
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        try:
            if data.get(str(chat.id)) == None:
                if conn:
                    text = "Backup comes from another chat, I can't return another chat to current chat *{}*".format(chat_name)
                else:
                    text = "Backup comes from another chat, I can't return another chat to this chat"
                return msg.reply_text(text, parse_mode="markdown")
        except:
            return msg.reply_text("There is problem while importing the data! Your data seems incompaitable! Maybe report that to support!")
        # Check if backup is from self
        try:
            if str(bot.id) != str(data[str(chat.id)]['bot']):
                return msg.reply_text("Backup from another bot that is not suggested might cause the problem, documents, photos, videos, audios, records might not work as it should be.\nTherefore Backup would not be restored!")
        except:
            pass
        # Select data source
        ...
        try:
            ...
            msg.reply_text("An error occurred while recovering your data. The process failed. If you experience a problem with this, please ask at @DraXRobots. My owner and community will be happy to help. Also, bugs report makes me even better!\nThank you!")
        ...
        if conn:
            text = "Backup fully restored on *{}*.".format(chat.name)
        else:
            text = "Backup fully restored"
        tgl = time.strftime("%H:%M:%S - %d/%m/%Y", time.localtime(time.time()))
        if MESSAGE_DUMP:
            try:
                bot.sendMessage(MESSAGE_DUMP, "*Successfully restored backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`".format(chat.title, chat_id, tgl), parse_mode=ParseMode.MARKDOWN)
            except BadRequest:
                pass
...
This module is still in beta! Not all data gets backed up!
Can be only used one time in 24 hours!
"""
