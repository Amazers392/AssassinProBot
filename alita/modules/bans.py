...
from alita import dispatcher, LOGGER, DEV_USERS, SUDO_USERS, BAN_STICKER
from alita.modules.disable import DisableAbleCommandHandler
from alita.modules.helper_funcs.chat_status import (bot_admin, user_admin, is_user_ban_protected, can_restrict,
                                                     is_user_admin, is_user_in_chat, connection_status)
from alita.modules.helper_funcs.extraction import extract_user_and_text
from alita.modules.helper_funcs.string_handling import extract_time
from alita.modules.log_channel import loggable, gloggable
...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Oh yeah, like I'd really ban myself, noob!")
        return log_message

    # dev users to bypass whitelist protection incase of abuse
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        message.reply_text("This user has immunity - I won't be able to ban them.")
        return log_message
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            # Do not reply
            message.reply_text('User Banned!', quote=False)
            return log
        else:
...
def unban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("How would I unban myself if I ain't there...?")
        return log_message
...
SBAN_HANDLER = CommandHandler("sban", sban, pass_args=True)
...
__mod_name__ = "Bans"
__handlers__ = [BAN_HANDLER, TEMPBAN_HANDLER, PUNCH_HANDLER, UNBAN_HANDLER, SELFUNBAN_HANDLER, PUNCHME_HANDLER, SBAN_HANDLER]
