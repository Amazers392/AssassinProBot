...
from assassin import dispatcher, OWNER_ID, SUDO_USERS, SUPPORT_USERS, DEV_USERS, WHITELIST_USERS, BAN_STICKER, spamfilters
from assassin.__main__ import GDPR
from assassin.__main__ import STATS, USER_INFO
from assassin.modules.disable import DisableAbleCommandHandler
from assassin.modules.helper_funcs.chat_status import user_is_gbanned
...
from assassin.modules.rextester.api import Rextester, CompilerError
from assassin.modules.rextester.langs import languages

from assassin.modules.sql.translation import prev_locale
...
CHU_STRINGS = (
    "Taare hai Asmaan me very very bright...Jhaat na jla bskd dekh le apni height.",
...
def thanos(bot: Bot, update: Update):
    ...
    if spam == True:
        return
    reply_text = update.effective_message.reply_to_message.reply_text if update.effective_message.reply_to_message else update.effective_message.reply_text
    reply_text(random.choice(THANOS_STRINGS))
...
def chu(bot: Bot, update: Update):
    ...
    if spam == True:
        return
    reply_text = update.effective_message.reply_to_message.reply_text if update.effective_message.reply_to_message else update.effective_message.reply_text
    reply_text(random.choice(CHU_STRINGS)) 
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if user.id == OWNER_ID:
        ...
        if user.id in SUPPORT_USERS:
            ...
            "Not quite a sudo user or a developer, but can still gban you off the map.")
             
        if user.id in SUDO_USERS:
             
            text += tld(chat.id, "\nThis person is one of my sudo users !" \

            "Don't mess with them kthnxbye :)")
                
        if user.id in DEV_USERS:

            text += tld(chat.id, "\nThis person is one of my DEVELOPERS! " \

            "They help to manage the bot and are more powerful than Sudos.")   
...
ECHO_HANDLER = CommandHandler("echo", echo, filters=CustomFilters.dev_filter)
