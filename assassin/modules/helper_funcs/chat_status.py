...
from assassin import DEL_CMDS, SUDO_USERS, WHITELIST_USERS, DEV_USERS
import assassin.modules.sql.admin_sql as admin_sql
...
def support_user(func):
    @wraps(func)
    def is_support_user_func(bot: Bot, update: Update, *args, **kwargs):
        user = update.effective_user
    ...
    return is_support_user_func


def whitelist_user(func):
    @wraps(func)
    def is_whitelist_user_func(bot: Bot, update: Update, *args, **kwargs):
        user = update.effective_user
        chat = update.effective_chat
