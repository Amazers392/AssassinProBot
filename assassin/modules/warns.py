...
from assassin import dispatcher, BAN_STICKER
from assassin.modules.disable import DisableAbleCommandHandler
from assassin.modules.helper_funcs.chat_status import is_user_admin, bot_admin, user_admin_no_reply, user_admin, \
    can_restrict
from assassin.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
from assassin.modules.helper_funcs.filters import CustomFilters
from assassin.modules.helper_funcs.misc import split_message
from assassin.modules.helper_funcs.string_handling import split_quotes
from assassin.modules.log_channel import loggable
from assassin.modules.sql import warns_sql as sql
from assassin.modules.sql import rules_sql as sqlr
...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    bot = dispatcher.bot
    rules = sqlr.get_rules(chat.id)
    if is_user_admin(chat, user.id):
        message.reply_text("Admin can't get warned shit!")
        return ""
    ...
    if num_warns >= limit:
        ...
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Remove warn", callback_data="rm_warn({})".format(user.id)), InlineKeyboardButton(text="Read\nRules", url="t.me/{}?start={}".format(bot.username, chat.id))]])
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if not is_user_admin(chat, int(user.id)):
            query.answer(text="You are not a admin kthnxbye.", show_alert=True)
            return ""
