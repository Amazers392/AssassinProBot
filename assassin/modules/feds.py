...
from assassin import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from assassin.modules.helper_funcs.handlers import CMD_STARTERS
from assassin.modules.helper_funcs.misc import is_module_loaded, send_to_list
...
from assassin.modules.helper_funcs.string_handling import markdown_parser
from assassin.modules.disable import DisableAbleCommandHandler

import assassin.modules.sql.feds_sql as sql
...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Federation creation failed! Keep in the mind that this rarely happened! Ask in @assassinAyaGroup for help!")
			return
