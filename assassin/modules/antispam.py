...
import assassin.modules.sql.antispam_sql as sql
from assassin import dispatcher, OWNER_ID, SUDO_USERS, DEV_USERS, SUPPORT_USERS, STRICT_ANTISPAM, GBAN_LOGS
from assassin.modules.helper_funcs.chat_status import user_admin, is_user_admin
from assassin.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from assassin.modules.helper_funcs.filters import CustomFilters
from assassin.modules.helper_funcs.misc import send_to_list
from assassin.modules.sql.users_sql import get_all_chats
...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text("I spy, with my little eye...war with a developer! Why are you guys turning on each other?")
        return
    ...
    if GBAN_LOGS:
        ...
        send_to_list(bot, SUDO_USERS + SUPPORT_USERS + DEV_USERS, "Gban complete! (User banned in {} chats)".format(gbanned_chats))
    ...
    try:
        bot.send_message(user_id, "You have been globally banned from all groups where I have administrative permissions. If you think that this was a mistake, you may appeal your ban here: @AssassinRoBotSupport", parse_mode=ParseMode.HTML)
    except:
...
def gmute(bot: Bot, update: Update, args: List[str]):
    ...
    send_to_list(bot, SUDO_USERS + SUPPORT_USERS + DEV_USERS, "gmute complete!")
...
def gkick(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in SUDO_USERS or int(user_id) in SUPPORT_USERS:
        ...
        return
    if int(user_id) in DEV_USERS:
        message.reply_text("OHHH! Someone's trying to gkick a developer! *Grabs popcorn*")
        return
